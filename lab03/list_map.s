...
map:
    ...
    # recurse
    ### YOUR CODE HERE ###
    jal map # Go to another map loop, save return address to ra. Need to use "jal" instead of "j",
            # because when all map loops finished, every loop needs to operate "done" to deallocate
            # memory in a reverse order. So we need to link/store the address of "done" to ra after
            # executing current loop before going to next loop.
    ...
    lw s0, 4(sp)
    lw ra, 8(sp)
    addi sp, sp, 12 # Restore register values in a reverse order, and deallocate memory.
